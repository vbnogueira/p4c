...
#define SB1(a)       \
    ...
    virtual void emitLicense(Util::SourceCodeBuilder *b, cstring l) const VB2(b,l)
    virtual void emitCodeSection(Util::SourceCodeBuilder *b, cstring n) const VB2(b,n)
    virtual void emitIncludes(Util::SourceCodeBuilder *b) const VB1(b)
    virtual void emitResizeBuffer(Util::SourceCodeBuilder *b, cstring bf, cstring o) const VB3(b,bf,o)
    virtual void emitTableLookup(Util::SourceCodeBuilder *b, cstring n, cstring k, cstring v) const VB4(b,n,k,v)
    virtual void emitTableUpdate(Util::SourceCodeBuilder *b, cstring n, cstring k, cstring v) const VB4(b,n,k,v)
    virtual void emitUserTableUpdate(Util::SourceCodeBuilder *b, cstring n, cstring k, cstring v) const VB4(b,n,k,v)
    virtual void emitTableDecl(Util::SourceCodeBuilder *b, cstring tn, TableKind k, cstring kt, cstring vt, unsigned int sz) const VB6(b,tn,k,kt,vt,sz)
    virtual void emitMain(Util::SourceCodeBuilder *b, cstring fn, cstring an) const VB3(b,fn,an)
    virtual cstring dataOffset(cstring b) const SB1(b)
    virtual cstring dataEnd(cstring b) const SB1(b)
    virtual cstring dataLength(cstring b) const SB1(b)
    virtual cstring forwardReturnCode() const SB0()
    virtual cstring dropReturnCode() const SB0()
    virtual cstring abortReturnCode() const SB0()
    virtual cstring sysMapPath() const SB0()
    virtual cstring packetDescriptorType() const SB0()
#undef VB1
#undef VB2
#undef VB3
#undef VB4
...
void StateTranslationVisitor::processMethod(const P4::ExternMethod *method) {
    ...
    if (decl == state->parser->packet) {
        if (method->method->name.name == p4lib.packetIn.extract.name) {
		    ...
		    ::P4::error(ErrorType::ERR_UNEXPECTED, "extract() with unexpected arg count %1%",
