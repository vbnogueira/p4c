...
EBPFType *EBPFTypeFactory::create(const IR::Type *type) {
    ...
    if (type->is<IR::Type_Boolean>()) {
        ...
        ::P4::error(ErrorType::ERR_UNSUPPORTED_ON_TARGET, "Type %1% not supported [factory create]", type);
    }
...
void EBPFScalarType::declare(CodeBuilder *builder, cstring id, bool asPointer) {
 if (isvariable)
  { builder->appendFormat("struct { u8 data[%d]; u16 curwidth; } %s",(width+7)>>3,id.c_str());
  }
        ...
        builder->appendFormat("struct internal_bit_%u %s%s",width,asPointer?"*":"",id.c_str());
    }
}
