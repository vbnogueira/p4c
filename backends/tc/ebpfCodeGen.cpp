...
void ControlBodyTranslatorPNA::processApply(const P4::ApplyMethod *method) {
    ...
    if (table->keyGenerator != nullptr) {
        ...
        builder->endOfStatement(true);
        builder->emitIndent();
	builder->appendLine("// XXX eBPF gets upset at memset(); let's");
	builder->appendLine("// hope __builtin_memset() does what we want.");
        ...
        for (auto c : table->keyGenerator->keyElements) {
            ...
            builder->emitIndent();
            if (!primitive) {
                builder->appendFormat("__builtin_memcpy/*B1*/(&(%v.%v), &(", keyname, fieldName);
                table->codeGen->visit(c->expression);
                builder->appendFormat("), %d)", scalar->bytesRequired());
            } else if (memcpy) {
                builder->appendFormat("__builtin_memcpy/*B2*/(&(%v.%v[0]), &(", keyname, fieldName);
                table->codeGen->visit(c->expression);
...
bool ControlBodyTranslatorPNA::preorder(const IR::BOr *e)
{
 return(arith_common(e,"|","bitor"));
}
