...
PARSER.add_argument(
    ...
    action='store_true',
    help=(
        "Do not remove temporary results for failing tests"
    ),
)
PARSER.add_argument(
    "-f",
...
def run_test(options, argv):
    ...
    tc = TCInfra(dirname, options, template)
...
def main(argv):
    ...
    options = Options()
    options.compiler_src_dir = testutils.check_if_dir(args.compiler_src_dir).as_posix()
    compiler = "./" + args.compiler

    options.compiler = testutils.check_if_file(compiler).as_posix()
