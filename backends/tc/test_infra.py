...
# We are reusing the ebpf backend's stf parsing function
from ebpfstf import parse_stf_file
...
class TCInfra:
    def __init__(self, tmpdir, options, template):
        ...
        self.virtme = Virtme("p4node", self.runtimedir, "iproute2-p4tc-pub",
                             self.spawn_script, self.post_boot_script,
                             options.verbose)
    ...
    def _load_rules(self, rules_file):
        result = testutils.exec_process(f"cp {rules_file} {self.outputdir}")
        if result.returncode != testutils.SUCCESS:
            return result.returncode

        return self.virtme.run_intros("{introspection}/" + f"{self.base_template}.rules")

    def load_rules(self):
        samples_dir = os.path.dirname(self.options.p4filename)
        rules_file = f"{samples_dir}/{self.base_template}.rules"
    ...
    def parse_stf_files(self, stffile):
        ...
        with open(stffile) as raw_stf:
            input_pkts, _, self.expected = parse_stf_file(raw_stf)
            result = self._send_pcap_files(input_pkts)
        return result
    ...
    def send_packets(self, testfile):
        return self.parse_stf_files(testfile)
    ...
    def run(self, testfile):
        ...
        result = self.send_packets(testfile)
        if result != testutils.SUCCESS:
            return result
