...
class NS:
    ...
    def _ns_exec(self, cmd: str, **extra_args: Any):
        ...
        if result.returncode != testutils.SUCCESS:
            testutils.log.error("Failed to run command in namespace %s",
                                self.ns_name)
        return result.returncode
...
class Virtme:
    def __init__(self, namespace, runtime_dir, iproute2_dir, spawn_script, post_boot_script,
                 verbose):
    ...
    def run_intros(self, cmd):
        intros_dir = f"{self.ns.mnt_dir}/generated"
    ...
    def run_tcpdump(self, filename, port):
        ...
        ssh_command = [
            "ssh", f"{self.user}@{self.host_ip}", "-i", f"{self.runtime_dir}/id_rsa",
