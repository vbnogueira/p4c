...
/*
 * These #defines exist so shift generation functions can be copied
 *  verbatim from the shifter test program.  See the comment generated
 *  near the top of gen_h() which talks about other #defines; similar
 *  remarks apply here.
 */
...
// See the comment on the #define of BUILDER, above.
#undef BUILDER
...
void SCAN_WIDTHS::gen_h(EBPF::CodeBuilder *bld) const
{
 ...
 //  this next comment.
 bld->appendLine("/*");
 bld->appendLine(" * These #defines exist so shift generation functions can be copied");
 bld->appendLine(" *  verbatim from the shifter test program.  By defining them suitably,");
 bld->appendLine(" *  the test programs can check for bits[] array overruns.");
 bld->appendLine(" *");
 bld->appendLine(" * We could edit them out when copying between the shift development");
 bld->appendLine(" *  code and here.  But the fewer edits are made in that process, the");
 bld->appendLine(" *  fewer opportunities for mistakes to creep in, so it seems to me to");
 bld->appendLine(" *  be worth a little effort to get the edit count down to zero.");
 bld->appendLine(" *");
 bld->appendLine(" * These aren't needed unless shift code is generated, but they are");
 bld->appendLine(" *  harmless if not, so we always produce them.");
 bld->appendLine(" */");
